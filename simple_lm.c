/* Simple LM implementation borrowed from libcifs */
/* Code in this file may be broken. It was not tested well. */
/* Code in this file may be broken due to additions by Aleksey Cherepanov. */
...
int main(void)
{
    /* "POLITIS" => 92936bce1c27859c aad3b435b51404ee */
    const char *test = "POLITIS";
    unsigned char out[200];
    size_t i;
    /* for (i = 0; i < 100000; i++) */
    auth_LMhash(out, test, strlen(test));
    for (i = 0; i < 16; i++) {
        if (i == 8)
            printf(" ");
        printf("%02x", out[i]);
