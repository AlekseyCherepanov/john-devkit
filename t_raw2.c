/* Experimental template for some sha3 candidates */
/*
...
static $type saved_key_length[$batch_size][$interleave][$vsize] $align;

#define saved_key_by_index(i, num) (saved_key[(i) / $vsize / $interleave][(num)][(i) / $vsize % $interleave][(i) % $vsize])

#define saved_key_length_by_index(i) (saved_key_length[(i) / $vsize / $interleave][(i) / $vsize % $interleave][(i) % $vsize])
...
#if $keccak
#define border_bits 0x06ULL
#else
#if $skein
/* %% actually there are no padding bits at all */
#define border_bits 0x00ULL
#else
#define border_bits 0x80ULL
#endif
#endif

#define make_if(shift, mask) \
        if (!(temp & (0xffULL << ((shift) * 8)))) { \
            saved_key_by_index(index, len / $size) = ((temp & (mask)) | (border_bits << ((shift) * 8))); \
            len += (shift); \
            goto key_cleaning; \
        }
        make_if(1, 0xffU);
...
#undef make_if
    ...
    saved_key_by_index(index, len / $size) = border_bits;
...
#  define LEN_POS 14
    ...
    saved_key_length_by_index(index) = len;
/* #if $skein */
/* #else */
...
static char *get_key(int index) {
    ...
    s = saved_key_length_by_index(index);

    printf(">> $formatu\n", s);

    for (i = 0; i < s / $size + 1; i++) {
        out[i] = saved_key_by_index(index, i);
        /* printf("$formatx  %zd %zd %zd\n", out[i], i, s); */
        printf("$formatx\n", out[i]);
    }
...
#define dk_v_read_input_length(num) (_mm_load_si128((__m128i *)saved_key_length[batch_index][(num) % $interleave]))
/* #define dk_read_input_length(num) (saved_key_length[batch_index][(num) % $interleave][0]) */
#define dk_read_input_length(num) (saved_key_length_by_index((num)))
...
#if $input_swap
#  define dk_read_input(num) (JOHNSWAP$bits(saved_key_by_index(index, (num))))
#else
#  define dk_read_input(num) (saved_key_by_index(index, (num)))
#endif

#undef dk_read_input_length
#define dk_read_input_length(num) (saved_key_length_by_index(index))
