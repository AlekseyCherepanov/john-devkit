/*
 * The template is based on truecrypt_fmt_plug.c from John the Ripper.
 */
 ...
 * Most optimizations by JimF were dropped.
...
#endif
	for(mi = 0; mi < count; mi += psalt->loop_inc)
	{
		/* unsigned char key[64]; */
		int ksz = strlen((char *)key_buffer[mi]);
#define dk_input_key() ((unsigned char *)key_buffer[mi])
#define dk_input_key_len() ksz
#define dk_input_salt() (psalt->salt)
#define dk_input_salt_len() 64
#define dk_input_salt2() (psalt->bin)
#define dk_input_salt2_len() 16
#define dk_output_bytes(s) memcpy(first_block_dec[mi], (s), 8)
$code
	}
	return count;
}
